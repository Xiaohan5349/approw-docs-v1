...
<ApiMethodSpec method="get" host="https://<your application domain name>.approw.com" path="/oidc/token" summary="Exchange token by client_secret_post" description="If you exchange token when configuring OIDC on the console and the authentication method is set to client_secret_post, then follow the following method to exchange token.">
...
Here is the [complete nodejs sample code](https://github.com/Approw/oidc-demo)：
