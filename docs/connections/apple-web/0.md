<IntegrationDetailCard title="在苹果开发者中心进行配置">

在 {{$localeConfig.brandName}} 控制台中配置 Sign in with Apple 社会化登录之前，你需要先在注册 Apple ID 并开通 Apple Developer，在 [Apple Developer Portal](https://developer.apple.com/account/#) 中注册你的 iOS 应用。完成此操作后，你可以使用从 Apple 获取到的 Team ID、Services ID、Key ID 和 Apple Signing Key 填入 {{$localeConfig.brandName}} Dashboard 配置表单。
...
2. 创建之后，记录下 **Key ID**，然后点击 **Download** 下载该**密钥(Signing Key)**：
