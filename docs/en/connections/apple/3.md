...
<ApiMethodSpec method="post" host="https://core.authing.cn" path="/connection/social/apple/callback?app_id=YOUR_APP_ID" description="use authorizationCode get user information">
...
{{$localeConfig.brandName}} The user information returned to the developer is the {{$localeConfig.brandName}} user standard field after processing, not the user field described in the WeChat document. For detailed user field definitions, please see: [User field definitions](../../user/profile.md)
