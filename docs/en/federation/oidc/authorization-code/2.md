...
<ApiMethodSpec method="get" host="https://<your application domain name>.approw.cn" path="/oidc/token" summary="Exchange token by client_secret_post" description="If you exchange token when configuring OIDC on the console and the authentication method is set to client_secret_post, then follow the following method to exchange token.">
...
Here is [complete nodejs sample code](https://github.com/approw/oidc-demo)：

```js
let code2tokenResponse
try {
  code2tokenResponse = await axios.post(
    'https://<your application domain name>.approw.cn/oidc/token',
    qs.stringify({
      code,
      client_id: oidcAppId,
...
```
curl --location --request POST 'https://<your application domain name>.approw.com/oidc/token' \
...
```json
{
  ...
  "scope": "openid profile approw_token email phone address offline_access",
  "token_type": "Bearer"
}
```
...
<ApiMethodSpec method="post" host="https://<your application domain name>.approw.com" path="/oidc/token" summary="Exchange token with none" description="If you set OIDC in the console and set the authentication method in exchange for token to none, then you do not need to pass the client_secret parameter when exchanging token. ">
<template slot="headers">
<ApiMethodParam name="Content-Type" type="string" required description="application/x-www-form-urlencoded" />
</template>
<template slot="formDataParams">
